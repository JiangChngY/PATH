...
## 项目简介
本工程演示 FreeRTOS 任务的动态创建、启动调度器以及运行时删除任务。启动任务创建 Task1、Task2、Task3 后删除自身，Task1/Task2 周期翻转 LED，Task3 负责按键检测。
...
## 学习重点
- 使用 `xTaskCreate()` 动态创建启动任务和业务任务。
- 使用 `vTaskStartScheduler()` 启动调度器。
- 使用 `vTaskDelete()` 删除指定任务和删除当前任务。
- 通过 `vTaskDelay()` 让任务进入阻塞态，观察调度切换。

## 运行现象
下载后打开串口，Task1/Task2/Task3 会打印运行信息并翻转 LED。按下 KEY1 后删除 Task1，LED1 和 Task1 的串口输出停止。
