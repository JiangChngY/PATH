...
## 项目简介
本工程演示 FreeRTOS 堆内存申请、释放和剩余堆空间查询。Task1 根据按键申请或释放 20 字节内存，并打印当前剩余堆空间。
...
## 学习重点
- 使用 `pvPortMalloc()` 申请 FreeRTOS 堆内存。
- 使用 `vPortFree()` 释放内存。
- 使用 `xPortGetFreeHeapSize()` 查看剩余堆空间。
- 理解 `heap_4.c` 的合并空闲块机制和内存泄漏风险。

## 运行现象
按 KEY1 申请 20 字节内存，按 KEY2 释放内存，串口输出当前剩余堆空间。
