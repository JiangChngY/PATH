...
## 项目简介
本工程演示计数型信号量。Task1 每次检测到 KEY1 就释放一次信号量，Task2 周期性获取信号量并打印当前同步结果。
...
## 学习重点
- 使用 `xSemaphoreCreateCounting()` 创建计数型信号量。
- 理解最大计数值和初始计数值。
- 使用 `xSemaphoreGive()` 增加计数。
- 使用 `xSemaphoreTake()` 消耗计数。

## 运行现象
连续按 KEY1 可以累积信号量计数；Task2 获取成功后串口输出提示。
